...
const App = () => {
const inisialtestate={
  Price:'',
  Quantity:'',
  Discount:''
}

const [{Price,Discount,Quantity},dispatch]=useReducer(ProducteReduser,inisialtestate)
...
const filterproductsPrise= Price > 0 ? products.filter(({price})=>price <= Price ):products;
const fiterDiscount=Discount>0 ? filterproductsPrise.filter(product=>product.discountedPrice > Discount):filterproductsPrise
//const filterproductsPrise= Price > 0 ?products.filter(({price}) => price <= Price):products;
  ...
  return (
    <>
      ...
      <span className='ms-20'>Filter</span>
      
      <input placeholder='set the price' className="border p-1 m-1" onChange={searchPrice}/>
      <input placeholder='set discount' className="border p-1 m-1 " onChange={searchDiscount}/>
      <input placeholder='set Qunatity' className="border p-1 m-1"onChange={searchQunatity} /><br/>

      <div className="flex flex-wrap gap-5 ms-20">
        {
        fiterDiscount?.length > 0 && fiterDiscount.map(product => (
          <div 
